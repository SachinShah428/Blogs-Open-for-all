...
const Form = ({ currentId, setCurrentId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    clear();
    localStorage.setItem('sachin', JSON.stringify( user.result._id ) );
    navigate('/world');
  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off"  className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post.title}"` : 'Creating a new Repository'}</Typography>
        ...
        <TextField name="message" variant="outlined" label="Content (paste here)" fullWidth multiline rows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} required />
